...
func main() {
	...
	server, err := su_net.CreateWSServer(*addr, func(conn *su_net.WSConn, dp *su_net.DataProtocol) {
		rq := &testpb.TestRQ{}
	...
	for i := 0; i < *clientCount; i++ {
		client, err := su_net.CreateWSClient(server.Addr, func(conn *su_net.WSConn, dp *su_net.DataProtocol) {
			rs := &testpb.TestRS{}
